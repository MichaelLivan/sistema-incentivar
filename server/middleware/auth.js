...
// ✅ CORREÇÃO 10: Middleware para verificar se pode confirmar atendimentos - VERSÃO FINAL CORRIGIDA
export const canConfirmSessions = (req, res, next) => {
  ...
  console.log('👤 [CONFIRM SESSIONS] Usuário completo:', {
    id: req.user?.id,
    name: req.user?.name,
  ...
  // ✅ CORREÇÃO PRINCIPAL: Lista completa e atualizada de tipos que podem confirmar atendimentos
  const allowedTypes = [
    'adm-geral',
    'adm-aba',
    'adm-denver', 
  ...
  console.log('🔍 [CONFIRM SESSIONS] Verificação detalhada de permissão:', {
    userType: req.user.type,
    ...
    isFinanceiro: req.user.type.startsWith('financeiro-'),
    isCoordenacao: req.user.type.startsWith('coordenacao-')
  });

  if (!canConfirm) {
    console.log('❌ [CONFIRM SESSIONS] Acesso negado para confirmação:', {
      userType: req.user.type,
      userSector: req.user.sector,
      allowedTypes,
      reason: 'Tipo de usuário não está na lista de permitidos'
    });
    return res.status(403).json({ 
      message: 'Você não tem permissão para confirmar atendimentos',
      userType: req.user.type,
      allowedTypes: allowedTypes,
      details: 'Apenas administradores, coordenadores e financeiro podem confirmar atendimentos'
    });
  }

  // ✅ VERIFICAÇÃO ADICIONAL: Para admins setoriais, verificar se o setor está definido
  if (req.user.type.startsWith('adm-') && req.user.type !== 'adm-geral') {
    if (!req.user.sector) {
      console.log('❌ [CONFIRM SESSIONS] Admin setorial sem setor definido:', {
        userType: req.user.type,
        sector: req.user.sector
      });
      return res.status(403).json({
        message: 'Admin setorial deve ter setor definido para confirmar atendimentos',
        userType: req.user.type,
        sector: req.user.sector,
        details: 'Entre em contato com o administrador geral para configurar seu setor'
      });
    }
    
    console.log('✅ [CONFIRM SESSIONS] Admin setorial com setor válido:', {
      userType: req.user.type,
      sector: req.user.sector
    });
  }

  console.log('✅ [CONFIRM SESSIONS] Permissão confirmada para confirmação de atendimentos:', {
    userType: req.user.type,
    userSector: req.user.sector,
...
export const debugPermissions = (req, res, next) => {
  console.log('🐛 [DEBUG] Informações completas de permissão:', {
    user: req.user ? {
      id: req.user.id,
      name: req.user.name,
      email: req.user.email,
  ...
  // ✅ Verificar permissões específicas se for rota de confirmação
  if (req.path.includes('/confirm') && req.method === 'PATCH') {
    console.log('🎯 [DEBUG] Rota de confirmação detectada - verificando permissões específicas');
    
    const allowedTypes = [
      'adm-geral', 'adm-aba', 'adm-denver', 'adm-grupo', 'adm-escolar',
      'coordenacao-aba', 'coordenacao-denver', 'coordenacao-grupo', 'coordenacao-escolar',
      'financeiro-ats', 'financeiro-pct'
    ];
    
    const hasPermission = req.user && allowedTypes.includes(req.user.type);
    
    console.log('🔍 [DEBUG] Análise de permissão para confirmação:', {
      hasUser: !!req.user,
      userType: req.user?.type,
      allowedTypes,
      hasPermission,
